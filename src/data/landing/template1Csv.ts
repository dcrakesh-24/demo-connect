/**
 * Template 1 CSV Data Loader
 * Loads configuration from CSV and converts to Template1 data structure
 */

import { loadCsvFile } from "@/lib/csvParser";
import type { LandingPageData, Feature, CaseStudy, BlogPost } from "./template1";

let template1DataCache: LandingPageData | null = null;
let template1Promise: Promise<LandingPageData> | null = null;

/**
 * Load Template1 data from CSV
 */
export async function loadTemplate1Data(): Promise<LandingPageData> {
  if (template1DataCache) {
    return template1DataCache;
  }

  if (template1Promise) {
    return template1Promise;
  }

  template1Promise = (async () => {
    try {
      const csvData = await loadCsvFile("/data/template1-config.csv");
      ...
      template1DataCache = data;
      return data;
    } catch (error) {
      console.error('Error loading Template1 CSV data:', error);
      throw error;
    }
  })();
...
/**
 * Get Template1 data (synchronous access to cache)
 */
export function getTemplate1Data(): LandingPageData | null {
  return template1DataCache;
}
